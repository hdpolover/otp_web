...
class M_login extends CI_Model {
  ...
  public function save_loginIP(){
    ...
    $data = array(
      ...
      'device'  => $this->agent->platform()." - ".$agent,
    );
    
    $this->db->insert('tb_login_ip', $data);
    return $this->db->affected_rows() == true;
  }
  
	public function get_userIPBLOKIR($id_user, $ip){
		$query 		= $this->db->query("SELECT * FROM tb_login_ip WHERE id_user = '$id_user' AND last_ip = '$ip' AND status = 0");
		if ($query->num_rows() > 0) {
      return TRUE;
    } else {
      return FALSE;
